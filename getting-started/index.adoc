...
The base domain that we input in the init step is used to create the DNS records for the SSO tools. http://{subdomain}.{base domain} is used to access the SSO tools. For example, http://jaeger.nsa2.com is used to access the Jaeger UI.

---
layout: documents
title: Building a Helm Chart for Multi-App Deployments
summary: "In this article, I explore how to structure a Helm chart that manages multiple applications—such as a frontend and backend—using sub-charts. You’ll learn how to set up the directory layout, configure values.yaml files to pass global or app-specific values, and deploy all components together. I also walk through best practices for organizing chart dependencies, writing IngressRoute manifests, and publishing the final chart to a container registry like AWS ECR. This setup is perfect for Kubernetes-native microservice environments where modular, repeatable deployments are key."
...
*YouTube Video Tutorial*: link:https://youtu.be/mGtIVDMaUkg[How to Build a Helm Chart with Multiple Applications (Frontend + Backend with Sub‑Charts)]

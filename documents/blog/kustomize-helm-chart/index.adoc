---
layout: documents
title: Using Kustomize With Helm Charts for Argo CD Applications
summary: "Managing Kubernetes applications across dev, staging, and production environments shouldn't feel like herding cats. Yet many teams struggle with configuration management, ending up with duplicated Application manifests, scattered values files, and no clear promotion path.
...
tags: "#HelmCharts #Kubernetes #DevOps #Microservices #GitOps #CloudNative #ServiceFoundry #HelmSubcharts #InfrastructureAsCode #AWS #ContainerOrchestration #PlatformEngineering #K8sTools"
...
Welcome! In this guide, we'll explore how to combine two powerful Kubernetes tools—Kustomize and Helm—to manage your applications across multiple environments using GitOps principles with Argo CD.
...
* 🔴 **Tight coupling**: Your configuration values are embedded in the Argo CD Application definition
* 🔴 **Poor reusability**: Hard to use the same Helm chart across dev, staging, and prod
* 🔴 **Version control mess**: Different values files scattered across multiple Application manifests
* 🔴 **Difficult to review**: Changes to values are mixed with application configuration
* 🔴 **No clear promotion path**: How do you promote changes from dev to staging to prod?
...
* ✅ **Clean separation**: Each environment gets its own directory with its own `values.yaml`
* ✅ **Reusability**: Use the same Helm chart, just point to different value files
* ✅ **GitOps-friendly**: All configuration lives in Git with clear directory structure
* ✅ **Easy promotion**: Copy or merge changes between environment directories
* ✅ **Better reviews**: Git diffs clearly show what changed per environment
...
* ✅ Simpler and more explicit—everything is in one place
* ✅ Easier to troubleshoot—you see the complete picture
* ✅ Standard Helm pattern—matches how most teams work
* ✅ GitOps friendly—clear diffs when values change
...
* ❌ Harder to validate—can't test with `helm template`
...
├── base/                # Empty (no shared resources for this use case)
├── dev/                 # Dev environment
│   ├── kustomization.yaml
│   ├── values-dev.yaml
...
│   ├── postgresql-applicationset.yaml        # Multi-environment manager
│   ├── postgresql-dev-application.yaml       # Dev application
│   ├── postgresql-prod-application.yaml      # Prod application
│   └── postgresql-staging-application.yaml   # Staging application
├── base/
│   └── kustomization.yaml                    # Empty base (not used here)
├── dev/                                       # Development environment
│   ├── kustomization.yaml                    # Dev Kustomize config
│   ├── postgresql-credentials-secret.yaml    # Dev secrets (sealed)
...
* ✅ **Clear separation**: Dev configuration lives in `dev/` directory
* ✅ **Easy reviews**: Git diffs show exactly what changed
* ✅ **Helm compatibility**: Leverages the full Helm chart ecosystem
* ✅ **Scalable**: Easy to add staging and prod environments next
...
1. `demo-postgresql-dev` → deploys from `demo-apps/postgresql-gitops/dev` → namespace `dev`
2. `demo-postgresql-staging` → deploys from `demo-apps/postgresql-gitops/staging` → namespace `staging`
3. `demo-postgresql-prod` → deploys from `demo-apps/postgresql-gitops/prod` → namespace `prod`
...
* ✅ **DRY principle**: One template instead of multiple duplicate manifests
* ✅ **Consistency**: All environments use the same configuration pattern
* ✅ **Easy to scale**: Adding a new environment? Just add one line to the `elements` list
* ✅ **GitOps-friendly**: The environment list is tracked in Git
...
GitOps means everything in Git. But Kubernetes Secrets are only base64-encoded—anyone with access to your repo can decode them:
...
* ✅ **Secure**: Secrets are encrypted, not just encoded
* ✅ **GitOps-compatible**: Safe to commit encrypted secrets to Git
* ✅ **Per-environment**: Different secrets for dev, staging, prod
* ✅ **Declarative**: Managed as code through CI/CD
...
* ✅ **Kustomize + Helm** gives you the best of both worlds: structure and flexibility
* ✅ **One values file per environment** may involve duplication, but it's clearer
* ✅ **Enable `--enable-helm`** in Argo CD's ConfigMap for Helm chart inflation
* ✅ **ApplicationSet** makes managing multiple environments a breeze
* ✅ **SealedSecrets** lets you safely store encrypted secrets in Git

**Next steps:**

* Try this pattern with your own Helm charts
* Explore Argo CD's ApplicationSet generators (Git, Cluster)
* Set up promotion workflows (dev → staging → prod)
...
* https://nsalexamy.github.io/service-foundry/pages/documents/blog/kustomize-helm-chart/
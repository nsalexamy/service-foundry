...
^.^a| icon:enterprise-apps-icon.svg[opts=inline, width=24] | Toggle to display only *Enterprise Applications*.

^.^a| image:icons/opensource-software-icon.svg[opts=inline, width=24]
...
^.^a| image:icons/uninstall-icon.svg[opts=inline, width=24]
...
^.^a| image:icons/collapse-icon.svg[opts=inline, width=24] | Hide File Tree.

^.^a| image:icons/expand-icon.svg[opts=inline, width=24] | Show File Tree.

^.^a| image:icons/refresh-icon.svg[opts=inline, width=24] | Refresh the file tree to reflect the current state from Git.

^.^a| image:icons/edit-icon.svg[opts=inline, width=24]
| Enable editing mode for the selected manifest file.

^.^a| image:icons/undo-icon.svg[opts=inline, width=24] |Discard edits and revert to the last committed version.
...
^.^a| image:icons/discard-icon.svg[opts=inline, width=24] | Discard all unsaved changes across files.

^.^a| image:icons/publish-icon.svg[opts=inline, width=24] | Commit and push changes to the Git repository (triggers Argo CD sync).

^.^a| image:icons/comment-icon.svg[opts=inline, width=24] | Add a Git commit message before pushing.

^.^a| image:icons/text-decrease-icon.svg[opts=inline, width=24] | Decrease font size in the file editor.

^.^a| image:icons/text-increase-icon.svg[opts=inline, width=24] | Increase font size in the file editor.
...
^.^a| image:icons/copy-files-icon.svg[opts=inline, width=32] | Copy the full path to the Argo CD application file for reference or external tooling.
^.^a| image:icons/edit-files-icon.svg[opts=inline, width=32] | Edit the Argo CD application manifest directly in the built-in file editor.
^.^a| image:icons/install-icon.svg[opts=inline, width=32] | Deploy the application to the cluster by creating the corresponding Argo CD application. Available only if the application is not yet installed.
^.^a| image:icons/uninstall-icon.svg[opts=inline, width=32] | Uninstall the application from the cluster and remove all associated Kubernetes resources. Available only if already installed.
^.^a| image:icons/discard-icon.svg[opts=inline, width=32] | Permanently delete the application manifest from the Git repository. This action cannot be undone.
|===
...
a| image:icons/discard-icon.svg[opts=inline, width=24] DELETE APPS |Delete selected GitOps application manifests from the repository. Irreversible.
a| image:icons/install-icon.svg[opts=inline, width=24] INSTALL APPS | Create Argo CD applications for selected manifests and deploy them to the cluster.
a| image:icons/uninstall-icon.svg[opts=inline, width=24] UNINSTALL APPS | Uninstall selected applications and remove their Kubernetes resources from the cluster.
|===
...
a| image:icons/file-icon.svg[opts=inline, width=24] View App Files | Open the file tree to browse and edit application manifests.
a| image:icons/install-icon.svg[opts=inline, width=24] Create ArgoCD App | Create a new Argo CD application to deploy the manifest to the Kubernetes cluster.
a| image:icons/uninstall-icon.svg[opts=inline, width=24] Uninstall ArgoCD App | Remove the Argo CD application and associated resources from the cluster.
a| image:icons/discard-icon.svg[opts=inline, width=24] DELETE APP |  Permanently delete the application manifest from the Git repository. This action is irreversible.
...
A subdomain like grafana with root domain nsa2.com will create a route: https://grafana.nsa2.com.

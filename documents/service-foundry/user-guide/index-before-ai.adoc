...
^.^a| image::icons/enterprise-apps-icon.png[opts=inline, width=32]
| Toggle button to filter and view only Enterprise Applications.

^.^a| image::icons/opensource-software-icon.png[opts=inline, width=32]
...
^.^a| image::icons/uninstall-icon.png[opts=inline, width=32]
...
^.^a| image::icons/copy-files-icon.png[opts=inline, width=32] | Copy the ArgoCD application file path to clipboard.
^.^a| image::icons/edit-files-icon.png[opts=inline, width=32] | Edit the ArgoCD application file directly on the console.
^.^a| image::icons/install-icon.png[opts=inline, width=32] | Install the selected application to the cluster. Enabled only if the application is not yet installed.
^.^a| image::icons/uninstall-icon.png[opts=inline, width=32] | Uninstall the selected application from the cluster. Enabled only if the application is already installed.
^.^a| image::icons/discard-icon.png[opts=inline, width=32] | Delete the selected application from the Git repository. This action cannot be undone.
|===

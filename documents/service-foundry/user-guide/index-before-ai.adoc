...
^.^a| image::icons/enterprise-apps-icon.svg[opts=inline, width=32]
| Toggle button to filter and view only Enterprise Applications.

^.^a| image::icons/opensource-software-icon.svg[opts=inline, width=32]
| Toggle button to filter and view only Open Source Software applications.

^.^a| image:icons/uninstall-icon.svg[opts=inline, width=24] Uninstall
| Button to uninstall the selected application from the cluster.

^.^a| image::icons/uninstall-icon.svg[opts=inline, width=32]
...
^.^a| image::icons/collapse-icon.svg[opts=inline, width=24] | Collapse the file tree view.

^.^a| image::icons/expand-icon.svg[opts=inline, width=24] | Expand the file tree view.

^.^a| image::icons/refresh-icon.svg[opts=inline, width=24] | Refresh the file tree view to see the latest state of the manifest files.

^.^a| image::icons/edit-icon.svg[opts=inline, width=24]
...
^.^a| image::icons/undo-icon.svg[opts=inline, width=24] | Discard changes made to the manifest files and revert to the last synced state.

^.^a| image::icons/save-icon.svg[opts=inline, width=24] | Save changes made to the manifest files. This will stage the changes for commit.

^.^a| image::icons/discard-icon.svg[opts=inline, width=24] | Discard all unsaved changes made to the manifest files.

^.^a| image::icons/publish-icon.svg[opts=inline, width=24] | Commit and push the changes to the Git repository. This will trigger Argo CD to sync the changes to the cluster.

^.^a| image::icons/comment-icon.svg[opts=inline, width=24] | Add a commit message when pushing changes to the Git repository.

^.^a| image::icons/text-decrease-icon.svg[opts=inline, width=24] | Decrease the font size of the manifest files for better readability.

^.^a| image::icons/text-increase-icon.svg[opts=inline, width=24] | Increase the font size of the manifest files for better readability.
...
^.^a| image::icons/copy-files-icon.svg[opts=inline, width=32] | Copy the ArgoCD application file path to clipboard.
^.^a| image::icons/edit-files-icon.svg[opts=inline, width=32] | Edit the ArgoCD application file directly on the console.
^.^a| image::icons/install-icon.svg[opts=inline, width=32] | Install the selected application to the cluster. Enabled only if the application is not yet installed.
^.^a| image::icons/uninstall-icon.svg[opts=inline, width=32] | Uninstall the selected application from the cluster. Enabled only if the application is already installed.
^.^a| image::icons/discard-icon.svg[opts=inline, width=32] | Delete the selected application from the Git repository. This action cannot be undone.
|===
...
^.^a| image:icons/discard-icon.svg[opts=inline, width=24] DELETE APPS | Delete the selected applications from the Git repository. This action cannot be undone.
^.^a| image:icons/install-icon.svg[opts=inline, width=24] INSTALL APPS | Install the selected applications to the cluster. Enabled only if the applications are not yet installed.
^.^a| image:icons/uninstall-icon.svg[opts=inline, width=24] UNINSTALL APPS | Uninstall the selected applications from the cluster. Enabled only if the applications are already installed.
|===
...
^.^a| image:icons/file-icon.svg[opts=inline, width=24] View App Files | View the manifest files of the application.
^.^a| image:icons/install-icon.svg[opts=inline, width=24] Create ArgoCD App | Create an Argo CD application to deploy the resources to the cluster. Enabled only if the application is not yet installed.
^.^a| image:icons/uninstall-icon.svg[opts=inline, width=24] Uninstall ArgoCD App | Uninstall the application from the cluster. This will remove all associated resources and delete the Argo CD application. Enabled only if the application is already installed.
^.^a| image:icons/discard-icon.svg[opts=inline, width=24] DELETE APP | Delete the application from the Git repository. This action cannot be undone.

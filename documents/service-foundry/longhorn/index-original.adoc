...
== How to connect to Kubernetes Nodes
...
TL;DR: Install Open-iscsi on All Nodes and enable and start the iscsid service. 
...
=== Container Runtime
.In SSH session, run the following command to check the container runtime
[source,shell]
----
$ containerd --version
containerd github.com/containerd/containerd/v2 2.1.5 fcd43222d6b07379a4be9786bda52438f0dd16a1
----

=== Kubernetes Version

.On local machine, run the following command to check the Kubernetes version
[source,shell]
----
$ kubectl version
...
By default, Amazon Linux 2023 kernels typically have NFSv4 enabled by default.

.Check if NFSv4, NFSv4.1, NFSv4.2 support is enabled
[source,shell]
----
...
.Verify NFSv4 client is installed
[source,shell]
----
$ mount.nfs -V

# Example output
mount.nfs: (linux nfs-utils 2.5.4)
----

.

//.In SSH session, run the following command to install nfs-utils
//[source,shell]
//----
...
Verify the filesystem supports file extents.

- ext4
- xfs

.In SSH session, run the following command to check the filesystem
[source,shell]
----
$ df -T
...
devtmpfs       devtmpfs   1984588        0   1984588   0% /
devtmpfs       devtmpfs   1984588        0   1984588   0% /dev
/dev/nvme0n1p1 xfs       8376724 1048576   7328148  13% /
----    

Make sure the filesystem supports file extents.

=== Utilities

By default, Amazon Linux 2023 has the utilities installed.
...
By default, Amazon Linux 2023 has the mount propagation enabled.



== Install Longhorn CLI

.In SSH session, run the following command to install Longhorn CLI
[source,shell]
----
$ LONGHORN_CLI_VERSION="v1.10.1"
$ curl -sSfL https://github.com/longhorn/cli/releases/download/${LONGHORN_CLI_VERSION}/longhornctl-linux-amd64 -o longhornctl
$ chmod +x longhornctl

---
layout: documents
title: "Using Longhorn - Snapshots, S3 Backups, and Restore Operations"
...
YouTube: https://youtu.be/yiAmRG74oto
...
1. Navigate to **Volumes** and Delete old volume that are now detached
2. Navigate to **Backup and Restore** → **Backups**
3. Under the **Volume** tab, find your PostgreSQL volume backups
4. Select the backup you want to restore (verify timestamp and labels)
5. Click **Restore**
6. In the restore dialog:
   * **Use Previous Name**: Check this box

7. Click **OK** to initiate the restore
8. Go back to **Volume** and select the target volume, and then, run 'Create PV/PVC' menu.
9. **Monitor progress**: The restore shows up in the volume list with a progress indicator
10. **Wait for completion**: Status changes to "Healthy" or "Bound" when done
